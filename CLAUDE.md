...
- `state/` — 永続化。`ExecutionStore`（IF）、`JsonExecutionStore`（実行ごとに 1 JSON）、`InMemoryExecutionStore`（テスト用）、`Durability`（保存を電源断から守る fsync。中身・改名・作成したディレクトリ階層の 3 段階を確定させる。原子性と耐久性の違いと段階ごとの失敗時の扱いは `docs/DESIGN.md` の「Record durability」が正本）。
...
- スタック形態は「CLI / バッチ」: 公開 URL・スクショは適用外、**端末操作の録画（asciinema / GIF）で代替**する。
- 録画対象の代表フロー 1 本: `examples/etl.yaml`（extract→transform→load→notify）の実行 — バッチ定義の読み込み → 依存グラフ解決 → 順次実行 → 実行結果/ログ出力まで。失敗時のリトライ・スキップ挙動を補足で含めてもよい。
- 録画は `docs/` 配下に置くか asciinema.org のリンクを README 冒頭のデモブロックに記載する。録画にはサンプル定義（`examples/`）のみ使い、実環境のパス・認証情報を写さない。
