...
オプション: `--state-dir <dir>`（状態保存先、既定 `.batch-state`）/ `-q`・`--quiet`（ジョブ別サマリ表を抑制）/ `list -n`・`list --limit <n>`（一覧の最大表示件数、既定 20・`0` 以下で全件）/ `--help` / `--version`。終了コード: 0=成功 / 1=バッチ失敗 / 2=検証エラー / 3=設定・IO エラー。
